...
import { Sparkles, Save, Trash2, ArrowLeft, Loader2, Download } from "lucide-react";
...
import { saveCoverLetter, deleteCoverLetter } from "../../actions";
...
export default function CoverLetterEditor({
    ...
    const [selectedResumeId, setSelectedResumeId] = useState<string>(coverLetter.resumeId || "none");
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            const values: CoverLetterValues = {
                ...
                resumeId: selectedResumeId === "none" ? undefined : selectedResumeId || undefined,
            };
    ...
    }, [coverLetter.id, title, companyName, jobTitle, jobDescription, tone, content, selectedResumeId]);
    ...
    const handleGenerate = useCallback(async () => {
        ...
        try {
            ...
            const result = await generateCoverLetter({
                ...
                resumeData: resumeData as Parameters<typeof generateCoverLetter>[0]["resumeData"],
                companyName: companyName || undefined,
                jobTitle: jobTitle || undefined,
                jobDescription: jobDescription || undefined,
                tone: tone as "professional" | "casual" | "enthusiastic" | "formal",
                                    ...
                                    <SelectContent>
                                        ...
                                        {resumes.map((r) => (
                                            <SelectItem key={r.id} value={r.id}>
                                                {r.title || `${r.firstName || ""} ${r.lastName || ""}`.trim() || "Untitled"}
                ...
                <div className="mx-auto max-w-2xl p-8">
                    <div className="mb-6 text-sm text-foreground">
                        {coverLetter.resumeId && (
                            <p className="text-muted-foreground">Re: {jobTitle || "Position"}</p>
                        )}
                    </div>
